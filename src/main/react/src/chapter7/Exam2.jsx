...
export default function Exam2( props ){
    ...
    // 1. useState 선언: comst [ 변수명 , set변수명 ] = useState(초기값);
    const[mode , setMode] = useState('both');   // 상태변수
                                                // vs. 차이점: 상태변수는 setXXX() 이용하여 값 수정시 함수/컴포넌트 재실행.
    let mode2 = 'both'                        // 일반변수
    mode2 = 'front'; // 일반변수: 값 변경 이후에 아무런 추가 기능이 없다. 바꿔봤자임.
    ...
    // 4. 상태에 따라 컴포넌트 그리기. 분가(if) 함. setMode가 실행돼서 상태가 변경되면 화면을 다시 그리므로('리랜더링(함수(컴포넌트)재호출)')
